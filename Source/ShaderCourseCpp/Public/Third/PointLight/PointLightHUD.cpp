...
void APointLightHUD::ResetLights()
{
	Lights.Empty();
	// 方法一
	/*
...
void APointLightHUD::LoadModel(UStaticMesh* InLoadMesh, FTransform InModelMatrix, FVector InKa, FVector InKd, FVector InKs, float InShiness)
{
...
UIShader* APointLightHUD::SpawnShader(EShaderType InShaderType, UMyModel* Model)
{
...
void APointLightHUD::SetGMaxIndex()
{
...
void APointLightHUD::SetViewMatrix()
{
	ViewMatrix = CameraTransform.Inverse().ToMatrixWithScale();
}

void APointLightHUD::SetPerspectiveMatrix()
{
...
void APointLightHUD::UpdateBuffer()
{
	BufferReset();
...
void APointLightHUD::BufferReset()
{
	//MyFrameBuffer.Reset();
...
void APointLightHUD::DrawTriangle(UMyModel* InModel, int32 InFaceNumber)
{
...
void APointLightHUD::ClipPlanes(TArray<FVertexOutput>& InVertexArray)
{
...
void APointLightHUD::ClipWithPlan(EClipPlane InClipPlane, TArray<FVertexOutput>& InVertexArray)
{
...
bool APointLightHUD::IsInsidePlane(EClipPlane InClipPlane, FVector4 InVertexPosCVV)
{
	switch (InClipPlane)
	{
...
float APointLightHUD::GetIntersecRatio(FVector4 InPreVertex, FVector4 InCurVertex, EClipPlane InClipPlane)
{
	switch (InClipPlane)
	{
	case EClipPlane::ClipWPlane:
...
FVector4 APointLightHUD::Vector4Lerp(FVector4 InPreVertex, FVector4 InCurVertex, float InAlpha)
{
	return InPreVertex + (InCurVertex - InPreVertex) * InAlpha;
}

bool APointLightHUD::IsBackface(TArray<FVector> InVertexPosNDC)
{
	// 1. 计算三角面的法向量
...
void APointLightHUD::TriangleSetup(const TArray<FVector>& InScreenPosArray, int32& OutXMin, int32& OutYMin, int32& OutXMax, int32& OutYMax)
{
...
void APointLightHUD::ComputeBarycentric(float InX, float InY, const TArray<FVector>& InScreenPosArray, float& OutAlpha, float& OutBeta, float& OutGamma)
{
...
bool APointLightHUD::IsInsideTriangle(float InAlpha, float InBeta, float InGamma)
{
...
int32 APointLightHUD::GetPixelIndex(int32 InPosX, int32 InPoxY)
{
	return InPoxY * FMath::TruncToInt32(GWidth) + InPosX;
}

float APointLightHUD::GetPixelDepth(int32 InPixelIndex)
{
